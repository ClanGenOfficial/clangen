...
After a long wait, ClanGen 0.13.0 is finally here! This update is absolutely packed. We have an updated main navigation, a new and improved relationship system, immersive ambiance and music, and much more! Read all about it below.
...
For a long time, ClanGen has been in need of some updated navigation UI! As we've added more and more features, they've mostly been tacked onto what already existed. This gets messy after a while and eventually the navigation became subpar both for players *and* for devs attempting to add to it. Our goal with this is to bring somewhat-hidden features to the surface and better consolidate information to maximize screen space. We hope this update helps you enjoy the game even more!
  ...
  - Supplies allows you to view two new windows: Freshkill and Herbs.
    - The Freshkill management window is taking on the old functionality of the Clearing. Here you can see all the hungry cats in the Clan, select and feed specific cats (or feed all cats at once!), and easily adjust the current Tactic selections (which have also gotten an upgrade.)
    - The Herb window is more bare bones, but will receive later upgrades. For now, it's just an easy way to access the Medicine Den log so that you can quickly see how the herb supply has changed.  
  - The Camp button has been removed. But fear not! The Camp screen is still here. It's now being treated as the "default" screen. You'll notice a new "Back" button on some screens, clicking this will take you back to the Camp! You can also click on the menu header that displays the Clan name to return to the Camp.
...
Relationships got a facelift! We wanted to improve the existing system to create space for more complexities in relationships, especially on the negative side of them.
  ...
  - Each value tier has a unique name that will be displayed. For example, the Mid Neg tier for Comfort will display as "fears" while the Mid Neg tier for Respect will display as "envies".
...
### Family Reactions to Litters
To complement the relationship update, extended family now gets to have a starting opinion on new additions! We hope this will create some fun dynamics and drama within families.
...
Our audio system was recoded from the ground up to allow us more freedom! This was a massive headache, but well worth it.
  ...
  - Unique ambient noise tracks added for all biomes. Certain camps and seasons have unique sound bites that will play at random intervals.
  - 6 new music tracks added. These will play at random intervals during the game.
...
![multi_accessory.png](../assets/0_13_changelog/multi_accessory.png)
///caption
...
### Status Rework
The status system has been entirely reworked! What's the "status system"? For a long time, a cat's "status" was a single string (code-talk for a length of text) denoting their rank or place in the world. A loner's rank was "loner", a warrior's rank was "warrior". This worked well for a long time! However, as ClanGen grew more complex it began to demand more information than a single string could provide. We made stopgap measures that allowed us to do more, but also tangled the code more. We realized that if we ever wanted to handle some of the complexities we *want* to handle, then we needed to change how status worked. The new system handles things in a more robust manner, allowing us to store significantly more information rather than just a single string.
...
### Localization Support
  ...
  - If you have interest in creating a localization, we'd like to direct you to our new documentation on the subject! https://clangen.io/docs/dev/writing/localization/
...
### User Interface
  ...
  - The Event display now features pagination in addition to the scrollbar. This gets rid of the issue large Clans would experience where the number of events make scrolling entirely impossible.
...
### Misc
  ...
  - Altered medicine cat apprentice odds to allow more than one medicine cat apprentice at once, boost odds of "logical" kits being selected, and lower odds of "unsuitable" kits being selected.
...
## **Technical**
  ...
  - Sprite generation was greatly refactored to reduce the amount of work required to add new sprites. For more information, especially on how to bring mods up to date, check out: https://clangen.io/docs/dev/art/cat-sprites/adding-new-sprites/

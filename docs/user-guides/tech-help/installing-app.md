...
A brief FAQ for questions that might want to be answered before the download process.
...
The application does not allow the player access to the game's source code. You will need to install the source version (see the installing UV tab).
...
If you are concerned with anti-virus preventing ClanGen from being downloaded or opened, ClanGen is an open source project on GitHub that has all its files and actions viewable. The game has an unregistered executable and an inactive built-in updater, so it's frequently auto-flagged as malicious when it's not.
...
- Windows 64: intended for Windows 7/8 devices (win7 installs are currently broken, install source), but newer Windows devices can also install this version
...
You should have a ClanGen folder inside that you'll open to access the clangen launcher. Double-click to run it.

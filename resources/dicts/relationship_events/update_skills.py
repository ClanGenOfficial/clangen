...
def change_skill(pa):
    ...
    if not isinstance(patrol_data, list):
        return
    ...
    if "tags" not in patrol_data[0]:
        return
    
    for _p in patrol_data:
        if "cat_skill" in _p:
            converted_skills = []
            if _p["cat_skill"] is not None:
                for _skill in _p["cat_skill"]:
                ...
                _p["cat_skill"] = final_skills
            
        if "other_cat_skill" in _p:
            converted_skills = []
            if _p["other_cat_skill"] is not None:
                for _skill in _p["other_cat_skill"]:
                    if _skill in conversion:
                        converted_skills.append([conversion[_skill][0].name, conversion[_skill][1]])
                ...
                _p["other_cat_skill"] = final_skills
                
        if "cat_negate_skill" in _p:
            converted_skills = []
            if _p["cat_negate_skill"] is not None:
                for _skill in _p["cat_negate_skill"]:
                    if _skill in conversion:
                        converted_skills.append([conversion[_skill][0].name, conversion[_skill][1]])
                ...
                _p["cat_negate_skill"] = final_skills

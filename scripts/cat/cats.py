...
from scripts.utility import get_personality_compatibility, event_text_adjust, update_sprite, \
    leader_ceremony_text_adjust, get_alive_status_cats
    ...
    def grief(self, body: bool):
        ...
        for cat in Cat.all_cats.values():
            ...
            if major_chance and not int(random() * major_chance):
                ...
                text = event_text_adjust(Cat, text, main_cat=self, random_cat=cat)
    ...
    def get_injured(self, name, event_triggered=False, lethal=True, severity='default'):
        ...
        if len(new_injury.also_got) > 0 and not int(random() * 5):
            ...
            if 'blood loss' in new_injury.also_got and len(
                    get_alive_status_cats(Cat, ["medicine cat", "medicine cat apprentice"], working=True)) != 0:
                clan_herbs = set()

...
class Cat:
    ...
    def init_faded(self, ID, status, prefix, suffix, moons, **kwargs):
        ...
        :param ID: Cat ID
        :param StatusEnum status: Cat status
    ...
    def gone(self):
        ...
        if self.status.is_leader() or self.status.is_warrior():
            ...
            self.status_change(StatusEnum.WARRIOR)
    ...
    def is_valid_mentor(self, potential_mentor: Cat):
        ...
        # Match jobs
        if (
                self.status.is_medcat_app()
                and not potential_mentor.status.is_medcat()
        ):
            return False
        ...
        if (
                self.status.is_mediator_app()
                and not potential_mentor.status.is_mediator()
        ):
            return False

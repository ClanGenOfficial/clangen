...
class FreshkillPile:
    ...
    def _update_needed_food(self, living_cats: List[Cat]) -> None:
        ...
        for queen_id, their_kits in queen_dict.items():
            ...
            if queen and (queen.outside or queen.status.is_exiled()):
                continue
        ...
        needed_prey = sum(
            [
                ...
                for cat in living_cats
                if not (cat.status.is_kit_any() or cat.status.is_outside_clan()) and not cat.outside
            ]
        )
        ...
        for feeding_status in FEEDING_ORDER:
            if feeding_status == "newborn":
                relevant_group = [
                    cat
                    for cat in living_cats
                    if cat.status.is_newborn() and cat not in fed_kits
                ]
            elif feeding_status == "kitten":
                relevant_group = [
                    cat
                    for cat in living_cats
                    if cat.status.is_kitten() and cat not in fed_kits
                ]
    ...
    def tactic_less_nutrition_first(
        ...
        for cat_id, v in sorted_nutrition.items():
            ...
            if "pregnant" not in cat.injuries and cat.not_working():
                ...
                if ration_prey and status.is_warrior():
    ...
    def feed_group(
        ...
        for cat in group:
            ...
            status = cat.status
            # check if this is a kit: if so, check if they are fed by the mother
            if status.is_kit_any() and fed_kits and cat in fed_kits:
                continue
    ...
    def feed_cat(self, cat: Cat, amount, actual_needed) -> None:
        ...
        elif ration and cat.status.is_warrior() and actual_needed != 0:
    ...
    def update_nutrition(self, living_cats: list) -> None:
        ...
        for cat in living_cats:
            ...
            if cat.ID in old_nutrition_info:
                ...
                factor = 3
                status_ = cat.status
                if cat.status.is_kit_any() or (
                        cat.moons > 114 and cat.status.is_elder()
                ):
                    factor = 2
    ...
    def add_cat_to_nutrition(self, cat: Cat) -> None:
        ...
        nutrition = Nutrition()
        factor = 3
        if cat.status.is_kit_any() or cat.status.is_elder():
            factor = 2

...
from scripts.clan.clan import HERBS
...
class Events:
    ...
    def handle_murder(self, cat, debug_force_murder=False):
        """ Handles murder """
        ...
        if (random.getrandbits(max(1, int(random_murder_chance))) == 1) or debug_force_murder:
            targets = [
                i
        ...
        if random.getrandbits(murder_capable) != 1 and not debug_force_murder:
            return
        ...
        if targets:
            ...
            if not int(random.random() * kill_chance) or debug_force_murder:
                print(

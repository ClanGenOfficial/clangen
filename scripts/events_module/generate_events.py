...
class GenerateEvents():
    ...
    def possible_death_events(self, cat_type, age):
        ...
        event_list.extend(self.generate_death_events(GenerateEvents.get_event_dicts("death", cat_type, "general")))

        if game.clan.biome in BIOME_TYPES:
            event_list.extend(self.generate_death_events(GenerateEvents.get_event_dicts("death", cat_type, game.clan.biome.lower())))

        if cat_type in ["apprentice", "deputy", "leader"]:
            event_list.extend(self.generate_death_events(GenerateEvents.get_event_dicts("death", "warrior", game.clan.biome.lower())))
        
        if cat_type not in ['kitten', 'leader']:
            event_list.extend(self.generate_death_events(GenerateEvents.get_event_dicts("death", "general", "general")))
            event_list.extend(self.generate_death_events(GenerateEvents.get_event_dicts("death", "general", game.clan.biome.lower())))

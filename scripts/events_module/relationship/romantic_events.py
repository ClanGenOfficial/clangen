...
class Romantic_Events():
    ...
    def prepare_relationship_string(mate_string, cat_from, cat_to):
        ...
        if "[m_c_mates]" in mate_string:
            mate_names = [str(cat_from.fetch_cat(mate_id).name) for mate_id in cat_from.mate if not cat_from.fetch_cat(mate_id).dead and not cat_from.fetch_cat(mate_id).outside]
        ...
        if "[r_c_mates]" in mate_string:
            mate_names = [str(cat_to.fetch_cat(mate_id).name) for mate_id in cat_to.mate if not cat_to.fetch_cat(mate_id).dead and not cat_to.fetch_cat(mate_id).outside]
            mate_name_string = mate_names[0]
            if len(mate_names) == 2:
                mate_name_string = mate_names[0] + " and " + mate_names[1]
            if len(mate_names) > 2:

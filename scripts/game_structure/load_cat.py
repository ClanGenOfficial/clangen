...
#moving the cat loading scripts to its own function so it can be reused for cat json importing
def json_load_cat(cat, convert = False):
    #will only load the conversion file once if its loading the full cat list, will read the dict again if importing an individual cat
    if not convert:
    ...
    if cat["white_patches"] in convert["old_creamy_patches"]:
        ...
        if 'white_patches_tint' in cat:
            ...
            if new_cat.white_patches in ['SEPIAPOINT', 'MINKPOINT', 'SEALPOINT']:
    ...
    if cat["pattern"] in convert["old_tortie_patches"]:
        # Convert old torties
...
def json_load():
    ...
    for i, cat in enumerate(cat_data):
        #each cat here
        try:
            new_cat = json_load_cat(cat, convert)

...
class Patrol(object):
    ...
    def add_possible_patrols(self):
        ...
        possible_patrols.extend([
            ...
            PatrolEvent(102, 'The patrol comes across a thunderpath', 'Your patrol crosses the thunderpath and can hunt on the other side',
                        ...
                        win_skills=['very smart', 'extremely smart']), PatrolEvent(103, 'p_l suggests this might be a good chance for the cats to practice teamwork',
            ...
            PatrolEvent(108, 'While helping gathering herbs, r_c stumbles upon a bush of red berries', 'The patrol tells r_c to stay away from the deathberries just in time',
            ...
            PatrolEvent(109, 'r_c notices a clanmate trapped in some brambles', 'r_c frees their clanmate',
                        ...
                        win_skills=['very smart', 'extremely smart']),
            PatrolEvent(110, 'r_c spots a large rabbit, but it is just over the border', 'r_c catches the rabbit without the enemy clan noticing',
        ...
        if game.clan.current_season == 'Newleaf':
            possible_patrols.extend([
                PatrolEvent(111, 'Your patrol notices new leaves and flowers starting to grow', 'The hunting is plentiful as new prey is born', 'With newleaf comes allergies...',
                            'Your patrol decides to head home early', 95, 10),
                PatrolEvent(112, 'r_c notes that it is a beautiful day outside, birds are singing and flowers are blooming', 'On days like these, patrolling is very pleasant',
                            'On days like these, cats are too lazy to patrol', 'Your patrol decides to head home early', 95, 10),
                PatrolEvent(113, 'The patrol approaches a deep ravine. There is a lot of prey here, but the ground is very slippery from newleaf rain',
                ...
                PatrolEvent(114, 'A large river divides the clan\'s territory and the water is high from newleaf rain. Should your patrol cross it?',
            ...
            possible_patrols.extend([
                PatrolEvent(120, 'It is extremely hot out today; r_c debates turning back home', 'They decide to power through the weather and the patrol is successful',
                            'r_c collapses from heat exhaustion', 'Your patrol decides to head home early to beat the heat', 50, 10),
                PatrolEvent(121, 'r_c spots a pond and debates if they should stop for a quick swim to cool off', 'They stop to swim for a few moments and feel rejuvenated',
                            'They get distracted while splashing in the water', 'r_c decides to continue with the patrol instead', 50, 10),
                PatrolEvent(122, 'The patrol comes across a strange twoleg object smelling slightly of smoke and prey',
                            ...
                            'The patrol can\'t seem to interpret the purpose of the object', 'The patrol decides to avoid the twoleg object', 50, 10,
                            win_skills=['smart', 'very smart', 'extremely smart']),

            ])
        elif game.clan.current_season == 'Leaf-fall':
            possible_patrols.extend([PatrolEvent(130, 'The leaves are starting to turn colors; the patrol know leaf-bare will be here soon', 'For now, hunting is still good',
                                                 'A chilly wind makes it difficult to hunt', 'The patrol is uneventful', 50, 10, win_skills=['great hunter', 'fantastic hunter']),
                                     PatrolEvent(131, 'r_c stalks a squirrel under a tree', 'They use the leaf-fall foliage to their advantage and catch the squirrel easily',
                                                 'Leaves crunch under their paws and the squirrel gets away', 'r_c decides to go after other prey instead', 50, 10,
                                                 win_skills=['fantastic hunter'])])
        elif game.clan.current_season == 'Leaf-bare':
            possible_patrols.extend([PatrolEvent(140, 'It starts snowing soon after the patrol sets out', 'Despite the snow, the patrol manages to hunt successfully',
                                                 'The prey must be hiding; the patrol catches nothing and is caught in a snowstorm',
                                                 'They decide to turn back and wait until the snow dies down', 50, 10, win_skills=['great hunter', 'fantastic hunter']),
                                     PatrolEvent(141, 'r_c thinks that it is too cold to patrol and that they should turn back',
                                                 ...
                                                 'They decide to turn back and wait for warmer weather', 50, 10,
                                                 win_skills=['good teacher', 'great teacher', 'fantastic teacher'])])

        if self.patrol_random_cat.status == 'apprentice' and len(self.patrol_cats) > 1:
            possible_patrols.extend([
                PatrolEvent(150, 'The patrol wants to hold a training session for r_c', 'r_c becomes more confident in their abilities after the training session',
                            'r_c is nervous and doesn\'t perform well', 'They decide to focus on the patrol instead', 50, 10)])
        ...
        if len(self.patrol_cats) > 1:
            possible_patrols.extend([
                ...
                PatrolEvent(201, 'The patrol finds a nice spot to sun themselves', 'The sunlight feels great and the cats have a successful patrol',
                            'The patrol doesn\'t get much done because of that', 'They decide to stay focused instead', 80, 10),
                PatrolEvent(200, 'Your patrol has a disagreement and look to p_l to settle the dispute', 'p_l manages to skillfully smooth over any disagreement',
                ...
                PatrolEvent(201, 'r_c admits that they had a vision from StarClan last night', 'The patrol talks them through the vision as they hunt',
                ...
                PatrolEvent(202, 'The patrol quickly devolves into ghost stories and everyone is on edge', 'Despite the tense mood, the patrol is successful',
                            'A branch snaps and the whole patrol runs back to camp', 'p_l quickly silences any talk about ghosts', 50, 10),
                PatrolEvent(203, 'r_c is tempted to eat the prey they just caught', 'They eat the prey without anyone noticing',
            ...
            if self.patrol_random_cat.status == 'warrior' or self.patrol_random_cat.status == 'apprentice':
                possible_patrols.extend([PatrolEvent(250, 'r_c admits that they have been training in the dark forest', 'The patrol manages to convince r_c to stop',
                                                     'The patrol isn\'t able to convince r_c to stop and a few nights later they are found dead in their nest',
                                                     'The patrol decides not to advise r_c what they should do', 50, 10,
                                                     win_skills=['great speaker', 'excellent speaker', 'strong connection to starclan']),
                                         PatrolEvent(251, 'r_c admits that they have been training in the dark forest', 'The patrol manages to convince r_c to stop',
                ...
                possible_patrols.extend([PatrolEvent(260, 'r_c admits that they don\'t think that they are a good deputy',
                                                     ...
                                                     win_skills=['great speaker', 'excellent speaker']),
                                         PatrolEvent(251, 'The patrol starts to doubt r_c\'s ability as the clan\'s deputy',
        ...
        if len(self.patrol_cats) == 1:
            possible_patrols.extend([
                PatrolEvent(400, 'r_c is nervous doing a patrol by themselves', 'They don\'t let their nerves get to them and continue the patrol successfully',
                            'They run back to camp, unable to continue', 'They continue the patrol but progress is slow', 40, 10, win_skills=['very smart', 'extremely smart']),
                PatrolEvent(401, 'Since r_c is alone, they debate taking a bite out of the freshkill they have just caught',
                            ...
                            'They shake the thought from their mind', 30, 10, win_skills=['smart', 'very smart', 'extremely smart']),
                PatrolEvent(402, 'While alone on patrol, r_c thinks about life', 'They find peace within themselves and enjoy the rest of the patrol',
                            'Their thoughts are plagued with bad memories', 'r_c decides to focus on the patrol', 40, 10, )])
            if self.patrol_cats[0].status == 'apprentice':
                possible_patrols.extend([PatrolEvent(450, 'r_c worries that an apprentice should not be out here alone', 'At least this is a good chance to learn the territory',
                                                     'r_c gets lost in the territory and doesn\'t learn anything', 'r_c turns back to camp, deciding that this is a bad idea', 40,
                                                     10, win_skills=['very smart', 'extremely smart']),
                                         PatrolEvent(451, 'r_c\'s mentor assesses them by sending them on a solo hunt', 'r_c catches a lot of prey and passes their assessment',
                                                     'Hunting is poor and r_c\'s mentor is disappointed', 'r_c asks their mentor to do the assessment some other time', 40, 10,
                                                     win_skills=['good hunter', 'great hunter', 'fantastic hunter']),
                                         PatrolEvent(452, 'r_c\'s mentor assesses them by sending them on a solo border patrol', 'r_c successfully marks the clan territory',
                                                     'r_c messes up the territory markings and almost starts a border skirmish',
                                                     'r_c asks their mentor to do the assessment some other time', 40, 10, )])

        # new cat patrols (not kit)
        possible_patrols.extend([PatrolEvent(500, 'Your patrol finds a loner who is interested in joining the clan', 'The patrol convinces the loner to join the clan',
                                             'The loner decides against joining', 'Your patrol decides not to confront the loner', 40, 10,
                                             antagonize_text='Your patrol drives the loner off of the territory',
                                             antagonize_fail_text='The loner is taken aback by their hostility and decides that clan life is not for them',
                                             win_skills=['great speaker', 'excellent speaker']), PatrolEvent(501, 'Your patrol finds a loner who is interested in joining the clan',
                                                                                                             'The patrol convinces the loner to join the clan and they bring with '
                                                                                                             'them a litter of kits', 'The loner decides against joining',
                                                                                                             'Your patrol decides not to confront the loner', 40, 10,
                                                                                                             antagonize_text='Your patrol drives the loner off of the territory',
                                                                                                             antagonize_fail_text='The loner is taken aback by their hostility '
                                                                                                                                  'and decides that clan life is not for them',
                                                                                                             win_skills=['great speaker', 'excellent speaker']),
                                 PatrolEvent(502, 'Your patrol finds a kittypet who is interested in joining the clan', 'The patrol convinces the kittypet to join the clan',
                                             'The description of clan life frightens the kittypet', 'Your patrol decides not to confront the kittypet', 40, 10,
                                             win_skills=['great speaker', 'excellent speaker']),
                                 PatrolEvent(503, 'r_c finds a wounded cat near the thunderpath', 'The patrol brings the cat back to camp. Once nursed back to health, '
                                                                                                  'the cat decides to join the clan',
                                             'As r_c inspects the cat, they find that they are already hunting '
                                             'with their ancestors', 'They leave the wounded cat alone', 40, 10, win_skills=['smart', 'very smart', 'extremely smart']),
        ...
        if self.patrol_random_cat.status == 'formerly a kittypet':
            possible_patrols.extend([PatrolEvent(520, 'r_c finds an old friend of their\'s from when they were a kittypet', 'r_c invites their friend to join the clan',
                                                 'r_c and their friend reminisce about old times', 'r_c says farewell to their friend and rejoins the patrol')])
    ...
    def handle_deaths(self):
        if self.patrol_event.patrol_id in [108, 113, 114, 120, 141, 250, 305, 307]:

...
class AllegiancesScreen(Screens):
    ...
    def get_allegiances_text(self):
        ...
        living_cats = [i for i in Cat.all_cats.values() if not (i.dead or i.outside.status.is_outsider())]
        living_meds = []
        living_mediators = []
        living_warriors = []
        living_apprentices = []
        ...
        outputs = []
        if game.clan.leader and not (game.clan.leader.dead or game.clan.leader.status.is_outsider()):
            outputs.append(
                [
        ...
        # Deputy Box:
        if game.clan.deputy and not (game.clan.deputy.dead or game.clan.deputy.status.is_outsider()):
            outputs.append(
                [

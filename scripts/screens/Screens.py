...
class Screens:
    ...
    def change_screen(self, new_screen):
        ...
        set_switch(Switches.cur_screen, new_screen)
    ...
    def menu_button_pressed(self, event):
        ...
        if event.ui_element == Screens.menu_buttons["events_screen"]:
            ...
            SaveCheck(
                get_switch(Switches.cur_screen), True, Screens.menu_buttons["main_menu"]
            )
            ...
            set_switch(
                Switches.moon_and_seasons_open,
                not get_switch(Switches.moon_and_seasons_open),
            )
    ...
    def mns_ui_offset(cls, x_shift, y_shift):
        ...
        if get_switch(Switches.cur_screen) != "camp screen":
            cls.menu_buttons.update(
                {
                    "dens": UIImageButton(
    ...
    def update_previous_next_cat_buttons(self):
        ...
        self.previous_cat_button.enable() if hasattr(  # pylint: disable=no-member
            self, "previous_cat"
        ) and self.previous_cat else self.previous_cat_button.disable()  # pylint: disable=no-member

        self.next_cat_button.enable() if hasattr(  # pylint: disable=no-member
            self, "next_cat"

...
class Screens:
    ...
    def move_focus(
        self, direction: FocusDirection, last_element: UIElement
    ) -> UIElement:
        """
        ...
        prior_row = None
        prior_col = None
        for index, row in enumerate(self.matrix_map):
            if last_element in row:
                prior_row = index
                prior_col = self.matrix_map[index].index(last_element)

        if not prior_row or not prior_col:
            # uh oh it must not be in the map and that's a problem!
            raise Exception(
                f"{last_element} not found in {self.name}'s matrix map. Use self.add_to_map() to add it. If this element shouldn't be interactable, then it was mistakenly given focus!"
            )
        ...
        # find a new row!
        if row_modifier:
            search_index = prior_row + row_modifier
            while search_index != prior_row and not new_row:
                # check if there are elements in the row
                if not any(self.matrix_map[search_index]):
                    search_index += row_modifier
                    # if we reach a bounds, we wrap around
                    if search_index < 0:
                        search_index = len(self.matrix_map)
                    if search_index > len(self.matrix_map):
                        search_index = 0
                else:
                    new_row = search_index

        # find a new column!
        if column_modifier:
            row_to_search = prior_row
            search_index = prior_col + column_modifier
            # first we need to prepare for a possible row change! if there are no valid elements in this row to change to,
            # then we'll have to search the next rows to see if there's any other elements available
            row_change_needed = True
            # if we find a valid element in the current row, then we'll switch row_change_needed to false and break that while loop
            # otherwise it breaks we find a new row with a valid element
            while row_change_needed and not new_row:
                while search_index != prior_col and not new_col:
                    row_without_cur_element = self.matrix_map[prior_row].copy()
                    row_without_cur_element.remove(last_element)
                    if not any(row_without_cur_element):
                        row_to_search = prior_row + 1

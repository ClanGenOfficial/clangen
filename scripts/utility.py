...
def init_inheritance(cat, cat_class, clan):
    ...
    if cat.ID not in clan.inheritance:
        clan.inheritance[cat.ID] = {
            "involved_no_cousins": [],
            "all_involved": [],
        }
    ...
    for relevant_id in parent_ids:
        # by blood
        relevant_cat = cat_class.fetch_cat(relevant_id)
        clan.inheritance[cat.ID]["parents"] = {}
        clan.inheritance[cat.ID][relevant_id] = {
            "type": RelationType.BLOOD,
            "name": str(relevant_cat.name)
        }
        clan.inheritance[cat.ID]["involved_no_cousins"].append(relevant_id)
        clan.inheritance[cat.ID]["all_involved"].append(relevant_id)

        # adoptive parents (mates of blood parents)
        for mate_id in relevant_cat.mate:
            relevant_cat = cat_class.fetch_cat(mate_id)
            clan.inheritance[cat.ID][relevant_id] = {
                "type": RelationType.ADOPTIVE,
                "name": str(relevant_cat.name)
            }
            clan.inheritance[cat.ID]["involved_no_cousins"].append(mate_id)
            clan.inheritance[cat.ID]["all_involved"].append(mate_id)

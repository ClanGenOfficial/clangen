...
def create_new_cat_block(
    ...
    for _tag in attribute_list:
        ...
        if Status(match.group(1)) in Status.list() \
                and Status(match.group(1)).is_inside_clan():
...
def ongoing_event_text_adjust(Cat, text, clan=None, other_clan_name=None):
    ...
    if "med_name" in text:
        kitty = choice(get_alive_status_cats(Cat, [Status.MEDCAT], working=True))
...
def event_text_adjust(
    ...
    # med_name
    if "med_name" in text:
        med = choice(get_alive_status_cats(Cat, [Status.MEDCAT], working=True))

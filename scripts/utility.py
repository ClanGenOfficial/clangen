    ...
    if not chosen_cat:
        ...
        for n_c in new_cats:
            ...
            for inter_cat in give_mates:
                if n_c == inter_cat or n_c.cat_id in inter_cat.mate:
                    continue
...
def get_highest_romantic_relation(
    ...
    for rel in relationships:
        if rel.romantic_love < 0:
            continue
        if exclude_mate and rel.cat_from.cat_id in rel.cat_to.mate:
            continue
    ...
    if "mates_with_pl" in filter_types:
        ...
        for cat in group:
            if cat.cat_id == patrol_leader.cat_id:
                continue
            if cat.cat_id not in patrol_leader.mate:
                return False
    ...
    for single_cat_from in cats_from:
        for single_cat_to in cats_to:
            ...
            if (
                ...
                or single_cat_to.cat_id in single_cat_from.mate
            ):

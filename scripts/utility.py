...
def get_alive_clan_queens(living_cats):
    ...
    for cat in living_kits.copy():
        ...
        parents = [
            ...
            for i in parents
            if cat.fetch_cat(i)
               and cat.fetch_cat(i).status.in_player_clan()
        ]
        if not parents:
            continue
...
def get_random_moon_cat(
    ...
    possible_r_c = list(
        filter(
            lambda c: c.status.in_player_clan()
...
def create_new_cat_block(
    ...
    if rank and not age:
        if rank not in [CatRank.APPRENTICE, CatRank.MEDIATOR_APPRENTICE, CatRank.MEDICINE_APPRENTICE]:
            age = randint(
...
def gather_cat_objects(
    ...
    for abbr in abbr_list:
        if abbr == "m_c":
            ...
            out_set.update(
                [x for x in Cat.all_cats_list if x.status.in_player_clan()]
            )
...
def ongoing_event_text_adjust(Cat, text, clan=None, other_clan_name=None):
    ...
    if "med_name" in text:
        kitty = choice(find_alive_cats_with_rank(Cat, [CatRank.MEDICINE_CAT], working=True))
...
def event_text_adjust(
    ...
    # med_name
    if "med_name" in text:
        med = choice(find_alive_cats_with_rank(Cat, [CatRank.MEDICINE_CAT], working=True))

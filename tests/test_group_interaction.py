...
class MainCatFiltering(unittest.TestCase):
    def test_main_cat_status_one(self):
        ...
        main_cat = Cat()
        main_cat.status = StatusEnum.WARRIOR
...
class Abbreviations(unittest.TestCase):
    def test_get_abbreviation_possibilities_all(self):
        # given
        main_cat = Cat(status=StatusEnum.WARRIOR)

        random1 = Cat(status=StatusEnum.WARRIOR)
        random2 = Cat(status=StatusEnum.WARRIOR)
        random3 = Cat(status=StatusEnum.WARRIOR)
    ...
    def test_get_abbreviation_possibilities_not_all(self):
        ...
        random1 = Cat(status=StatusEnum.WARRIOR)
        random2 = Cat(status=StatusEnum.WARRIOR)
        random3 = Cat(status=StatusEnum.MEDCAT)
...
class OtherCatsFiltering(unittest.TestCase):
    def test_relationship_allow_true(self):
        # given
        parent = Cat()
        main_cat = Cat(parent1=parent.ID, status=StatusEnum.WARRIOR)
        random1 = Cat(parent1=parent.ID, status=StatusEnum.WARRIOR)
        random2 = Cat(status=StatusEnum.WARRIOR)
        abbreviations_cat_id = {
            "m_c": main_cat.ID,
            "r_c1": random1.ID,
